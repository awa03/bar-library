...
namespace loadingBar {
    ...
    namespace premade {
      ...
      static animation none({animation::frame("")});

      static animation loading({animation::frame(" Loading"), animation::frame(" Loading."), animation::frame(" Loading.."), animation::frame(" Loading...")});

      static animation loading_no_move({animation::frame(" Loading   "), animation::frame(" Loading.  "), animation::frame(" Loading.. "), animation::frame(" Loading...")});

      static animation fade_in_bar({animation::frame("▒"), animation::frame("█")});
      static animation dice({animation::frame(symbol::dice_one), animation::frame(symbol::dice_two), animation::frame(symbol::dice_three), animation::frame(symbol::dice_four), animation::frame(symbol::dice_five), animation::frame(symbol::dice_six)});
